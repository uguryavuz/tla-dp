...
EXTENDS Integers, Sequences, DP, Reals
CONSTANT Eps, Q
    ...
    v_delta = 0;
  {
    L1: while (0 < Len(l1)) { await ((0 < Len(l1)) = (0 < Len(l2)));
          await ((Len(l1) % Q = 0) = (Len(l2) % Q = 0)); if (Len(l1) % Q = 0) {
            with (res \in AbsLap(Eps, c1 + Head(l1), c2 + Head(l2), v_eps, v_delta)) {
            ...
            with (res \in AbsLap(Eps, Head(l1), Head(l2), v_eps, v_delta)) {
              x1 := res[1] || x2 := res[2] || v_eps := res[3] || v_delta := res[4]
            };
...
\* BEGIN TRANSLATION (chksum(pcal) = "223b2509" /\ chksum(tla) = "b0d14f08")
...
L1 == /\ pc = "L1"
      /\ IF 0 < Len(l1)
            THEN /\ ((0 < Len(l1)) = (0 < Len(l2)))
                 ...
                 /\ IF Len(l1) % Q = 0
                       THEN /\ \E res \in AbsLap(Eps, c1 + Head(l1), c2 + Head(l2), v_eps, v_delta):
                                 /\ v_delta' = res[4]
                                 /\ v_eps' = res[3]
                                 /\ x1' = res[1]
                                 /\ x2' = res[2]
                            ...
                            /\ /\ r1' = Append(r1, next1')
                               /\ r2' = Append(r2, next2')
                       ELSE /\ \E res \in AbsLap(Eps, Head(l1), Head(l2), v_eps, v_delta):
                                 /\ v_delta' = res[4]
                                 /\ v_eps' = res[3]
                                 /\ x1' = res[1]
                                 /\ x2' = res[2]
